...
class EmbeddingGeneratorNet(nn.Module):
    """Two-Tower network for embedding generation.
    ...
    Output:
        - user_emb: (N, D) via get_user_tower_embed
        - item_emb: (K, D) via get_item_tower_embed
        - scores:   (N, K) via forward
    """
    ...
    def forward(self, user_features, item_idxes=None):
        """Compute scores for user against items.
        ...
        """
        user_emb = self.get_user_tower_embed(user_features)
        item_emb = self.get_item_tower_embed(item_idxes)
        scores = torch.matmul(user_emb, item_emb.T)
        return scores

    def get_user_tower_embed(self, user_features):
        """Compute user embeddings from user features.

        Args:
            user_features: dict with click_items (N, T) and optional side info.
    ...
    def get_item_tower_embed(self, item_idxes=None):
        """Compute item tower embeddings.

        Args:
            item_idxes: (K,) item indices. If None, compute for all items.

        Returns:
            item_emb: (K, D) or (num_items+1, D)
        """
